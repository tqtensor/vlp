...
trait TokenizerTransformerParams extends Params {
  val convertPunctuation: Param[Boolean] = new BooleanParam(
    this,
    "convert punctuations to PUNCT",
    "convert punctuations"
  )
  val convertNumber: Param[Boolean] =
    new BooleanParam(this, "convert number to [NUM]", "convert numbers")
  val toLowercase: Param[Boolean] =
    new BooleanParam(this, "convert text to lowercase", "make text lowercase")
  val splitSentences: Param[Boolean] = new BooleanParam(
    this,
  ...
  def setConvertPunctuation(value: Boolean): this.type =
    set(convertPunctuation, value)
...
class TokenizerTransformer(override val uid: String)
    extends UnaryTransformer[String, String, TokenizerTransformer]
    with TokenizerTransformerParams
    with DefaultParamsWritable {

  val tokenizerBr: Option[Broadcast[Tokenizer]] = Some(
    SparkSession.getActiveSession.get.sparkContext.broadcast(new Tokenizer())
  )
...
object TokenizerTransformer
    extends DefaultParamsReadable[TokenizerTransformer] {
  final val punctuations = Array(
    ",",
    ".",
    ":",
    ";",
  ...
  def convertPunct(token: String): String =
    if (punctuations.contains(token)) "PUNCT" else token.replaceAll(",", ".")
