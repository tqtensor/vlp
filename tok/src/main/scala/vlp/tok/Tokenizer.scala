...
object Tokenizer extends Serializable {
  ...
  val bricks = List[Brick](
    ...
    Brick(
      "road",
      raw"""\b([Qq]uốc lộ|[Tt]ỉnh lộ|[Hh]uyện lộ|[Đđ]ường)\s+(\d+\p{Lu}+|\p{Lu}+\d+)\b""".r,
      5
    ),
    Brick(
      "docNo",
      raw"""\b([Ll]uật|[Nn]ghị quyết|[Cc]ông văn|[Qq]uyết định|[Nn]ghị định|[Tt]hông tư|[Tt]ờ trình|[Tt]hông báo|[Cc]hỉ thị|[Vv]ăn bản)(\s+(số))?\s+(\d+[\p{Lu}-]*/)+[\p{Lu}-]*\b""".r,
      5
    ),
    ...
    Brick(
      "url",
      raw"""(((\w+)\://)+[a-zA-z][\-\w]*\w+(\.\w[\-\w]*)+(/[\w\-]+)*(\.\w+)?(/?)(\?(\w+=[\w%]+))*(&(\w+=[\w%]+))*|[a-z]+((\.)\w+)+)""".r,
      3
    ),
    Brick(
      "name",
      raw"""\b(\p{Lu}\p{Ll}*)([\s+_&\-]?(\p{Lu}\p{Ll}*))+\b""".r,
      2
    ),
    Brick("allCapital", raw"""\b[\p{Lu}]{2,}([\s_][\p{Lu}]{2,})*\b""".r, 1),
    Brick(
      "date1",
      raw"""\b(([12][0-9]|3[01]|0*[1-9])[-/.](1[012]|0*[1-9])[-/.](\d{4}|\d{2})|(1[012]|0*[1-9])[-/.]([12][0-9]|3[01]|0*[1-9])[-/.](\d{4}|\d{2}))\b""".r,
      1
    ),
    Brick("date2", raw"""\b(1[012]|0*[1-9])[-/](\d{4}|\d{2})\b""".r, 1),
    Brick(
      "date3",
      raw"""\b([12][0-9]|3[01]|0*[1-9])[-/](1[012]|0*[1-9])\b""".r,
      1
    ),
    Brick(
      "date4",
      raw"""\b([Nn]gày)(\s+)\d+(\s+)tháng(\s+)\d+(\s+)năm(\s+)(\d+)\b""".r,
      1
    ),
  ...
  val lexicon = Source
    .fromInputStream(
      Tokenizer.getClass.getResourceAsStream("/lexicon.txt"),
      "UTF-8"
    )
    .getLines()
    .toSet
...
class Tokenizer extends Serializable {
  def segment(
    ...
    if (forward) segmentForward(syllables, verbose);
    else segmentBackward(syllables, verbose)
  }
  ...
  def segmentForward(
    ...
    if (syllables.size >= 1) {
      ...
      if (m < n) {
        val right =
          segmentForward(syllables.slice(m + 1, syllables.size), verbose)
        result ++= right
      }
    }
    result.toList
  ...
  /** Splits and merges two tokens [name, lower], for example ["Bộ Quốc",
    * "phòng"] => ["Bộ", "Quốc phòng"].
    * @param tokens
    * @param verbose
    */
  ...
  /** Tokenizes a text into tokens; each token is a tuple (index, shape, word).
    * @param text
    *   a text, usually a sentence
    * @param forward
    *   use forward maximal matching instead of backward.
    * @param verbose
    * @return
    *   a list of tokens
    */
  def tokenize(
    ...
    val nonSpaceTokens =
      tokens.filter(t => t._3.trim.size > 0 && t._3.trim != "\u00A0")
    // re-index the tokens, merge syllables with underscore and return result
    nonSpaceTokens.zipWithIndex.map(pair =>
      (pair._2 + 1, pair._1._2, pair._1._3.replaceAll("\\s+", "_"))
    )
  }
  ...
  def tokenizeOne(text: String): List[List[(Int, String, String)]] = {
    ...
    val different =
      backwardTokens.zip(forwardTokens).exists(a => a._1._3 != a._2._3)
    if (different) List(backwardTokens, forwardTokens);
    else List(backwardTokens)
  }

  /** Tokenizes a sequence of texts into tokens; each token is a tuple (index,
    * shape, word).
    * @param texts
    *   a sequence of texts
    * @param parallel
  ...
  /** Tokenizes a input text file and write the result to an output text file or
    * to the console.
    * @param inputPath
    * @param outputPath
    * @param parallel
  ...
  def process(
    ...
    val texts =
      Source.fromFile(inputPath).getLines().filter(_.trim.nonEmpty).toList
