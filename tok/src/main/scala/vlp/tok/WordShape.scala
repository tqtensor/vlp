...
object WordShape {
  ...
  val capitalizedWithPeriod =
    """(\p{javaUpperCase}\p{javaLowerCase}?\.)+\p{javaUpperCase}?""".r
  ...
  val containsHyphen =
    """[\p{javaUpperCase}\p{javaLowerCase}]+\-[\p{javaUpperCase}\p{javaLowerCase}]+""".r
  val number = """[\+\-]?([0-9]*)?[0-9]+([\.,]\d+)*""".r
  val percentage = """[\+\-]?([0-9]*)?[0-9]+([\.,]\d+)*%""".r
  val date =
    """(\d\d\d\d)[-/\.](\d?\d)[-/\.](\d?\d)|((\d?\d)[-/\.])?(\d?\d)[-/\.](\d\d\d\d)""".r
  val name =
    """(\p{javaUpperCase}\p{javaLowerCase}+)([-_&,\s]+(\p{javaUpperCase}\p{javaLowerCase}+))+""".r
  val code = """\d+\p{javaUpperCase}""".r
  val weight =
    """[\+\-]?([0-9]*)?[0-9]+([\.,]\d+)*(Kg|kg|lbs|kilograms|kilogram|kilos|kilo|pounds|pound)""".r
  val punctuation = """[.,?!;:\"…/”“″=^▪•<>&«\])(\[\u0022\u200b\ufeff+-]+""".r
  val email = """(\w[-._%:\w]*@\w[-._\w]*\w\.\w{2,3})""".r
  val url =
    """(((\w+)\://)+[a-zA-z][\-\w]*\w+(\.\w[\-\w]*)+(/[\w\-~]+)*(\.\w+)?(/?)(\?(\w+=[\w%~]+))*(&(\w+=[\w%~]+))*|[a-z]+((\.)\w+)+)""".r
  val date1 =
    """\b(([12][0-9]|3[01]|0*[1-9])[-/.](1[012]|0*[1-9])[-/.](\d{4}|\d{2})|(1[012]|0*[1-9])[-/.]([12][0-9]|3[01]|0*[1-9])[-/.](\d{4}|\d{2}))\b""".r
  val date2 = """\b(1[012]|0*[1-9])[-/](\d{4}|\d{2})\b""".r
  val date3 = """\b([12][0-9]|3[01]|0*[1-9])[-/](1[012]|0*[1-9])\b""".r
  val date4 =
    """\b([Nn]gày)([\s_]+)\d+([\s_]+)tháng([\s_]+)\d+([\s_]+)năm([\s_]+)(\d+)\b""".r
  ...
  def shape(word: String): String = {
    word match {
      ...
      case date1(_*) =>
        "date" // should goes after number (but we want to recognize 12.2004 as a date.)
      case date2(_*)       => "date"
      case date3(_*)       => "date"
      case date4(_*)       => "date"
      case time(_*)        => "time"
  ...
  def main(args: Array[String]): Unit = {
    ...
    words.foreach(e => {
      println(
        e + " => shape = " + WordShape.shape(e) + ", normalize = " + normalize(
          e
        )
      )
    })
